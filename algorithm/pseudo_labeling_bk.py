...
class Pseudo_Labeling(object):
    ...
    def __init__(self, unlabelled_data, x_test,y_test, num_iters=5,upper_threshold = 0.8,
            fraction_allocation=1,lower_threshold = None,num_XGB_models=0,verbose = False):
        ...
        xgb = self.get_XGB_model(param_list)
    ...
    def get_XGB_model(self,param_list): # we create the XGB model depending on multiclass or multi-label setting
        if self.IsMultiLabel==False:
            return XGBClassifier(**param_list,use_label_encoder=False)
        else:
            return MultiOutputClassifier(XGBClassifier(**param_list,use_label_encoder=False))
    ...
    def evaluate_performance(self):
        # evaluate_performance the classification performance for multi-class classification
