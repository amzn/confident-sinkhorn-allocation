...
for ii, data in enumerate(all_data):
    ...
    try:  
        ...
        strFile=folder+"/CSA_TotalEnt_{:d}_{:s}.npy".format(ii,_datasetName[ii])
    ...
    if len(SLA_noconfidence)>0:
        ...
        print("SLA {:.2f} ({:.1f})".format(np.mean(SLA_noconfidence[:,-1]),np.std(SLA_noconfidence[:,-1])))
    ...
    if len(CSA_TotalVar)>0:
        ...
        print("CSA_TotalVar {:.2f} ({:.1f})".format(np.mean(CSA_TotalVar[:,-1]),np.std(CSA_TotalVar[:,-1])))
