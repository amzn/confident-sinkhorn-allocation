...
for upper_threshold in threshold_list:
    ...
    for tt in range(nRepeat):
        ...
        #         upper_threshold,lower_threshold,
        #         num_iters=NumIter,num_XGB_models=M,
        ...
        # pseudo_labeller = csa_totalentropy(copy.copy(xgb),
        #         x_unlabeled,x_test,y_test, 
        #         upper_threshold,lower_threshold,
        #         num_iters=NumIter,
        #         verbose = True
    ...
    strFile=save_folder+"/flex_{:d}_{:s}_{:.2f}".format(ii,_datasetName[ii],upper_threshold)
    if np.sum(AccFlex)>0:
        np.save(strFile, AccFlex)
    
    strFile=save_folder+"/pl_{:d}_{:s}_{:.2f}".format(ii,_datasetName[ii],upper_threshold)
    if np.sum(AccPL)>0:
    ...
    # strFile=save_folder+"/CSA_TTest_{:d}_{:s}_M_{:d}".format(ii,_datasetName[ii],M)
    ...
    # strFile=save_folder+"/Sinkhorn_ori_{:d}_{:s}_{:d}_{:d}".format(ii,_datasetName[ii],FromIndex,ToIndex)
    # if np.sum(AccSinkhornOrig)>0:
    #     np.save(strFile, AccSinkhornOrig)
